...
void monitorTOFSensor(VL53L4CX& sensor, int motorPin) {
  ...
  // Check for new data with timeout handling
  #if defined(WIRE_HAS_TIMEOUT)
    unsigned long startTime = millis();
    do {
      status = sensor.VL53L4CX_GetMeasurementDataReady(&NewDataReady);
      if (millis() - startTime > 1000) {
        Serial.println("Timeout occurred while waiting for new data");
        // Re-initialize TOF sensor
        // Initialize I2C bus.
        DEV_I2C.begin();
        // Configure VL53L4CX satellite component.
        sensor_vl53l4cx_sat.begin();
  ...
  if (NewDataReady) {
   // Serial.println("After status check");
    ...
    Serial.println("Timeout: Data not ready after 1 second");
    // Re-initialize the TOF sensor here if needed
    // sensor.init(); 
  }
}
