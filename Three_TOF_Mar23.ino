...
void setup() {
  ...
  if (! sensor_vl53l4cx_sat.begin()) {   
    Serial.println("Couldn't find TOF #1");
    while (1) delay(1);
  }

  // Initialize TOF #2 at pin A3 
  tcaselect(2);
  if (! sensor_vl53l4cx_sat_2.begin()) {   
    Serial.println("Couldn't find TOF #2");
    while (1) delay(1);
  }

  // Initialize TOF #1 at pin A5
  tcaselect(4);
  if (! sensor_vl53l4cx_sat_3.begin()) {   
    Serial.println("Couldn't find TOF #3");
    while (1) delay(1);
  }
  ...
  sensor_vl53l4cx_sat.VL53L4CX_StartMeasurement(); // start measurement 
